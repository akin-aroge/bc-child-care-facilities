...
facility_time_path_local = root_dir.joinpath("./data/raw/ccof_facilities_and_spaces_over_time.csv")
...
def get_raw_data(update=False):
    ...
    if not update:
        try:
            ...
            facility_df, locs_df = _pull_data_from_web()
    else:
        facility_df, locs_df = _pull_data_from_web()
...
def enhance_facility_loc_df(facility_loc_df:pd.DataFrame) -> pd.DataFrame:
    ...
    df = (
        ...
        .assign(region=facility_loc_df['CITY'].str.lower().apply(lambda x: C.CITIES_REGIONS_MAP.get(x, 'other')))
    )
    return df
...
def get_dashboard_data():
    ...
    facility_df, locs_df = get_raw_data(update=False)
