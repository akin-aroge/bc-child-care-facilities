...
def plot_distribution(data: pd.DataFrame, group_col_name:str, top_n:int=10) -> matplotlib.figure.Figure:
    ...
    temp_df = (
        ...
        .loc[:, [group_col_name]]
        .groupby(group_col_name).apply(lambda x: x.count() * 100.0 / len(data.query("IS_DUPLICATE == 'N'")))
    ...
    try:
        sns.barplot(data=temp_df.reset_index()[:top_n], x=group_col_name, y=agg_col_name, color='#1f77b4', ax=ax)
...
def plot_distribution_by_region(data: pd.DataFrame,  top_n:int=10) -> matplotlib.figure.Figure:
    """plot the distribution of child care facilities"""
    ...
    temp_df = (
        data
        .query("IS_DUPLICATE == 'N'")
        .assign(region=data['CITY'].str.lower().apply(lambda x: cities_regions_map.get(x, 'other')))
        .loc[:, [group_col_name]]
        .groupby(group_col_name).apply(lambda x: x.count() * 100.0 / len(data.query("IS_DUPLICATE == 'N'")))
        .rename(columns={group_col_name:agg_col_name})
        .sort_values(by=agg_col_name, ascending=False)
