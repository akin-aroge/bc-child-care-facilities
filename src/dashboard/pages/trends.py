...
def main():
    ...
    df = dash_utils.filter_facility_df(year_filter=select_year,
...
def service_filter():
    ...
    select_service = st.sidebar.selectbox("Select service to see trned:",
                                          options=services,
                                          index=0)
...
def region_filter():
    ...
    select_region = st.sidebar.multiselect(label="select regions to view:",
                                           options=regions,
                                           default=regions)
...
def sec_kpi():
    ...
    n_facilities, delta_facilities = dash_utils.get_last_facility_count_n_change(col_to_count=cols_to_view[0])
    n_spaces, delta_spaces = dash_utils.get_last_facility_count_n_change(col_to_count=cols_to_view[1])
